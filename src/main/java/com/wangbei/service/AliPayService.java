...
public class AliPayService {
	...
	public String pay(Integer user, TradeTypeEnum tradeTypeEnum, Double amount) {
		...
		try {
			...
			e.printStackTrace();
			throw new ServiceException(ExceptionEnum.UNKNOW_EXCEPTION, new Object[] { "" }, e);
		}
	}
	...
	public String callback(Map<String, String> params) throws AlipayApiException {
		...
		if (flag) {
			...
			if (status.equals("TRADE_SUCCESS")) {
				// orderService.updateOrderStatus(outTradeNo, tradeNo,
				// OrderStatusEnum.SUCCESS, new Date());
	...
	@Transactional
	public String h5Pay(Integer user, TradeTypeEnum tradeTypeEnum, Double amount) {
		// 创建订单和交易 且订单状态为未支付
		String tradeNo = TradeService.generateTradeNo();
		orderService.generateOrder(user, PaymentTypeEnum.AliPay, amount, tradeNo, tradeNo);
		tradeService.paymentTrade(tradeNo, user, tradeTypeEnum, (int) (amount * 10));
		...
		request.setBizModel(model);
		request.setNotifyUrl(AlipayConfigConstant.NOTIFY_URL);
		request.setReturnUrl(AlipayConfigConstant.RETURN_URL);
		request.putOtherTextParam("app_pay", "Y");
		try {
