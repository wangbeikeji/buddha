...
public class UserBuddhistNoteController {
	...
	public Response<Page<UserBuddhistNote>> pagePublicByType(int type, int page, int limit) {
		return new Response<>(userBuddhistNoteService
				.pagePublicUserBuddhistNoteByType(BuddhistNoteTypeEnum.getByIndex(type), page, limit));
	}

	@GetMapping("/pageUserByType")
	@ApiOperation(value = "获取用户佛录列表", notes = "其中type传值说明如下：1祈福录，2行善录，3忏悔录")
	public Response<Page<UserBuddhistNote>> pageUserByType(int type, int page, int limit) {
		AuthUserDetails authUserDetails = SecurityAuthService.getCurrentUser();
